...
# 产物在 build/libs/qshop-forge-1.20.1-1.5.0.jar
```
...
交易设置界面、编辑子商店界面和物品浏览器界面也支持 F8 调试；配置中的 `screens.trade_settings`、`screens.tab_settings` 和 `screens.item_picker` 保存这些界面的独立组件偏移。
编辑子商店界面中可分别调整面板、各输入行、图标按钮组、显示条件勾选框以及删除/保存/取消按钮。
...
加载优先级为内置默认样式、资源包样式、本地 `config/qshop_layout.json` 中的布局偏移。资源包遵循 Minecraft 的资源包优先级，最上层资源包中的 `assets/qshop/style.json` 会覆盖下层资源包。
